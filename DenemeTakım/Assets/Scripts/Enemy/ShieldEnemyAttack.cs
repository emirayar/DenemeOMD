...
public class ShieldEnemyAttack : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (distanceToPlayer <= chaseRange && !isAttacking && HasLineOfSight())
        {
            ...
            if (direction.x < 0)
                transform.rotation = Quaternion.Euler(0, 180, 0); // Saða bakýyorsa
            else
                transform.rotation = Quaternion.Euler(0, 0, 0); // Sola bakýyorsa
        ...
        if (distanceToPlayer <= attackRange && !isAttacking && HasLineOfSight())
        {
            ...
            animator.SetTrigger("ShieldAttackA");
        ...
        if (damageGiven > 0)
        {
            ...
            animator.SetTrigger("PlayerDamageTaken");
    ...
    private void GiveDamage()
    {
        ...
        {
            animator.SetTrigger("PlayerDamageTakenB"); // Ýkinci saldýrý animasyonunu tetikle
        }
